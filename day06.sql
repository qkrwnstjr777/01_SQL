...
SELECT upper ('sql is cooooooool~!!!') "재밌어요!"
 FROM dual
;
-- 재밌어요!
--SQL IS COOOOOOOOL~!!!
...
-- str의 글자길이, 글자의 byte 를 계산하여 숫자로 출력

SELECT length('hello, sql') as "글자 길이"
 FROM dual
 ;
 --결과 10 : 특수기호, 공백도 글자 길이로 계산된다.
...
-- sql is SQLISCOOOOOOOOOOL~~!! 에서 sql 만 추출 

SELECT substr('SQL is cooooooooool ~~!!',1,3)
 FROM dual
; 

-- SQLISCOOOOOOOOOOL~~!! 에서 is 만 추출
SELECT substr('SQL is cooooooooool ~~!!' ,5,2)
 FROM dual
; 

-- SQLISCOOOOOOOOOOL~~!! 에서 ~~!! 만 추출
...
7369	SMITH
*/
-- 문제 : 비교값 '  smith ' 일때 SMITH의 정보를 조회 하시오
...
-- NULL 값을 대체하는 '매니저없음' 은 문자타입의 데이터이므로
-- 변환시 mgr 컬럼의 타입과 불일치 하기 때문에 
-- 실행시 구문 오류가 발생 

-- ==> 해결법 e.mgr을 문자 타입으로 변경 
SELECT e.empno
        ,e.ename
        ,nvl(e.mgr || '' ,'매니저 없음') "매니저 사번"
 FROM emp e 
...
---------------- 3) 날짜함수 : 날짜와 관련된 출력/ 날짜의 더하기 빼기 등이 연산을
--                      할 수 있는 기능을 제공
...
SELECT to_char(sysdate, 'FMYY-MM-DD')  "오늘 날짜"  -- 19-5-24
FROM dual
;
...
SELECT to_char(sysdate, 'YYYY-MM-DD HH24:MI:SS')  "오늘 날짜 시분초"  -- 2019-05-24 15:30:55
 FROM dual
;

SELECT to_char(sysdate, 'YYYY-MM-DD AM HH:MI:SS')  "오늘 날짜 시분초"  --2019-05-24 오후 03:43:23
 FROM dual
;
-- AM 패턴문자를 조합하면 오전/오후 구별됨

-- 날짜 값과 숫자의 연산 : + , - 연산자를 사용
-- 지금으로부터 10일 후
SELECT sysdate + 10    "10일 뒤 " -- 19/06/03
FROM dual
;

SELECT sysdate - 10    "10일 전 " -- 19/05/14
FROM dual
;

SELECT sysdate + 10/24   "10시간후" -- 19/05/25
FROM dual
;

-- 10시간 후를 시/분/초 까지 출력

SELECT to_char(sysdate + 10/24, 'YYYY-MM-DD HH24:MI:SS')   "10시간후" -- 2019-05-25 01:47:12
FROM dual
;

------------- 1 MONTHS_BETWEEN(날짜1, 날짜2) :
-- 두 날짜 사이의 달의 차이
-- 각 직원이 입사한 날로부터 현재까지 몇개월을 근무 했는지 조회
...
SELECT e.empno
        ,e.ename
        ,ROUND (MONTHS_BETWEEN(sysdate, e.hiredate),1)     " 근속 개월 수"  -- 하나의 값으로 생각하고 중첩
FROM emp e
;
...
-- 날짜 + 숫자/24 : 숫자 만큼의 시간을 더해서 날짜를 구함
-- ADD_MOUNTHS(날짜, 숫자) : 숫자 만큼의 달을 더하여 날짜를 구함
...
------------- 4. ROUND,TURNC : 날짜에 대한 반올림, 버림 연산
SELECT round(sysdate) "현재시간"
FROM dual
;

-- 문제 : 현재 시간에서 반올림 된 날짜를 시/분/초 까지 출력
-- 힌트 : to_char() 시분초 까지 출력할수 있는 날짜/시간 패턴을 조합 
SELECT   to_char(ROUND (sysdate),'YYYY-MM-DD AM HH24:MI:SS')
FROM dual
;
-- 2019-05-25 오전 00:00:00
